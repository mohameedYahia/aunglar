...
export class LandDetailsComponent {
  ...
  futureInstallments = computed<Installment[]>(() => {
    ...
    if (currentLand.mechanism === 'مزاد علني') {
        ...
        // Year 1 after grace
        installments.push({ period: 'السنة الأولى - دفعة 1', amount: annualAmount * 0.20, dueDate: formatDate(lastDueDate), status: lastDueDate < today ? 'مستحق السداد' : 'قادم'});
        lastDueDate.setMonth(lastDueDate.getMonth() + 6);
        installments.push({ period: 'السنة الأولى - دفعة 2', amount: annualAmount * 0.40, dueDate: formatDate(lastDueDate), status: lastDueDate < today ? 'مستحق السداد' : 'قادم'});
        lastDueDate.setMonth(lastDueDate.getMonth() + 6);
        installments.push({ period: 'السنة الأولى - دفعة 3', amount: annualAmount * 0.40, dueDate: formatDate(lastDueDate), status: lastDueDate < today ? 'مستحق السداد' : 'قادم'});
        
        // Subsequent 24 years
        for(let i = 2; i <= 25; i++) {
            annualAmount *= 1.02; // 2% increase
            lastDueDate.setMonth(lastDueDate.getMonth() + 6);
            installments.push({ period: `السنة ${i} - دفعة 1`, amount: annualAmount * 0.50, dueDate: formatDate(lastDueDate), status: lastDueDate < today ? 'مستحق السداد' : 'قادم'});
            lastDueDate.setMonth(lastDueDate.getMonth() + 6);
            installments.push({ period: `السنة ${i} - دفعة 2`, amount: annualAmount * 0.50, dueDate: formatDate(lastDueDate), status: lastDueDate < today ? 'مستحق السداد' : 'قادم'});
        }
